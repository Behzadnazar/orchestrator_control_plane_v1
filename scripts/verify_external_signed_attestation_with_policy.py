...
VERIFY_ROOT = OPERATIONS_ROOT / "external_attestation_policy_verification"
...
def parse_iso_zoned(value: str) -> datetime:
...
def key_within_window(entry: dict, verification_time: datetime) -> tuple[bool, str]:
    not_before_raw = str(entry.get("not_before", "")).strip()
    not_after_raw = str(entry.get("not_after", "")).strip()

    if not not_before_raw or not not_after_raw:
        return False, "missing_validity_window"

    not_before = parse_iso_zoned(not_before_raw)
    not_after = parse_iso_zoned(not_after_raw)
...
def build_markdown_report(result: dict, output_dir: Path) -> str:
    lines: list[str] = []
    lines.append("# External Signed Attestation Verification with Key Policy")
    lines.append("")
    lines.append(f"- Runtime label: `{result['runtime_label']}`")
    lines.append(f"- Verification time (UTC): `{result['verification_time_utc']}`")
    lines.append(f"- Verification status: **{result['verification_status']}**")
    lines.append(f"- Output directory: `{rel(output_dir)}`")
    lines.append("")
    lines.append("## Registry Policy")
    lines.append("")
    ...
    lines.append(f"- Key usage valid: **{result['checks']['registry_key_usage_valid']}**")
    lines.append(f"- Key within validity window: **{result['checks']['registry_key_within_window']}**")
    lines.append(f"- Window verdict: `{result['derived']['window_verdict']}`")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify external-key signed attestation against registry policy, validity window, and revocation state.")
    parser.add_argument("--runtime-label", required=True)
    parser.add_argument("--registry-path", required=True)
    parser.add_argument("--verification-time", default="")
    parser.add_argument("--label", default="")
    args = parser.parse_args()

    runtime_label = args.runtime_label.strip()
    registry_path = Path(args.registry_path).resolve()
    label = args.label.strip() or f"{runtime_label}__policy_verification"

    if not runtime_label:
        raise SystemExit("runtime-label must not be empty.")

    if args.verification_time.strip():
        verification_time = parse_iso_zoned(args.verification_time.strip())
    else:
    ...
    usage_valid = bool(registry_entry and registry_entry.get("usage") == "attestation_signing")
    window_ok = False
    window_verdict = "no_registry_entry"
    if registry_entry is not None:
        window_ok, window_verdict = key_within_window(registry_entry, verification_time)
    ...
    checks = {
        ...
        "registry_key_usage_valid": usage_valid,
        "registry_key_within_window": window_ok,
    ...
    result = {
        ...
        "verification_type": "external_signed_attestation_policy_verification",
        "runtime_label": runtime_label,
        "verification_time_utc": verification_time.isoformat(),
    ...
    json_path = output_dir / "external_signed_attestation_policy_verification.json"
    md_path = output_dir / "external_signed_attestation_policy_verification.md"
    digest_path = output_dir / "external_signed_attestation_policy_verification_digest.json"
    ...
    print("=" * 72)
    print("EXTERNAL SIGNED ATTESTATION POLICY VERIFICATION")
    print("=" * 72)
    print(f"RUNTIME LABEL : {runtime_label}")
    print(f"STATUS        : {verification_status}")
    print(f"WINDOW VERDICT: {result['derived']['window_verdict']}")
