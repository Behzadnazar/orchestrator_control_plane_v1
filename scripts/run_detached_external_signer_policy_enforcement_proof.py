...
STATE_DIR = ROOT / "state" / "detached_external_signer_policy"
...
def scenario_paths(base_dir: Path, scenario_name: str) -> dict[str, Path]:
    ...
    return {
        ...
        "allowed_payload": scenario_dir / "allowed_execution_receipt_payload.json",
        "allowed_signature": scenario_dir / "allowed_execution_receipt.sig",
        "allowed_verification": scenario_dir / "allowed_execution_receipt_verification.json",
        "allowed_tampered_payload": scenario_dir / "allowed_execution_receipt_tampered_payload.json",
        "allowed_tamper_verification": scenario_dir / "allowed_execution_receipt_tamper_verification.json",
        "disallowed_type_payload": scenario_dir / "disallowed_type_payload.json",
        "disallowed_type_response": scenario_dir / "disallowed_type_sign_response.json",
        "disallowed_class_payload": scenario_dir / "disallowed_class_payload.json",
        "disallowed_class_response": scenario_dir / "disallowed_class_sign_response.json",
        "disallowed_target_payload": scenario_dir / "disallowed_target_payload.json",
        "disallowed_target_response": scenario_dir / "disallowed_target_sign_response.json",
        "runtime_report": scenario_dir / "detached_signer_policy_runtime_report.json",
...
def build_manifest(
    *,
    ...
    return {
        "manifest_version": 1,
        "proof_type": "detached_external_signer_policy_enforcement_proof",
        "proof_label": label,
...
def simulate_policy_enforcement_execution(
    *,
    label: str,
    scenario_name: str,
    key: ScenarioKey,
    ...
    allowed_payload = build_payload(
        label=label,
        scenario_name=scenario_name,
        key=key,
        gate_decision=gate_decision,
        scenario_verification_time=scenario_verification_time,
        request_name="allowed_request",
        request_type="execution_receipt",
        payload_class="runtime_evidence",
        target_path="proof/allowed/output.txt",
        content="allowed-content",
    )
    write_text(paths["allowed_payload"], canonical_json(allowed_payload) + "\n")

    allowed_response = send_sign_request(
        signer_proc,
        payload_path=paths["allowed_payload"],
        signature_path=paths["allowed_signature"],
        request_id="allowed_request",
    )
    allowed_verify = openssl_verify(paths["allowed_payload"], key.public_key_path, paths["allowed_signature"])
    write_json(paths["allowed_verification"], allowed_verify)

    allowed_tampered = dict(allowed_payload)
    allowed_tampered["payload_class"] = "tampered_runtime_evidence"
    write_text(paths["allowed_tampered_payload"], canonical_json(allowed_tampered) + "\n")
    allowed_tamper_verify = openssl_verify(
        paths["allowed_tampered_payload"],
        key.public_key_path,
        paths["allowed_signature"],
    )
    write_json(paths["allowed_tamper_verification"], allowed_tamper_verify)

    disallowed_type_payload = build_payload(
        label=label,
        scenario_name=scenario_name,
        key=key,
        gate_decision=gate_decision,
        scenario_verification_time=scenario_verification_time,
        request_name="disallowed_type_request",
        request_type="admin_config_snapshot",
        payload_class="runtime_evidence",
        target_path="proof/allowed/output.txt",
        content="wrong-type",
    )
    write_text(paths["disallowed_type_payload"], canonical_json(disallowed_type_payload) + "\n")
    disallowed_type_response = send_sign_request(
        signer_proc,
        payload_path=paths["disallowed_type_payload"],
        signature_path=paths["scenario_dir"] / "disallowed_type_payload.sig",
        request_id="disallowed_type_request",
    )
    write_json(paths["disallowed_type_response"], disallowed_type_response)

    disallowed_class_payload = build_payload(
        label=label,
        scenario_name=scenario_name,
        key=key,
        gate_decision=gate_decision,
        scenario_verification_time=scenario_verification_time,
        request_name="disallowed_class_request",
        request_type="execution_receipt",
        payload_class="admin_control",
        target_path="proof/allowed/output.txt",
        content="wrong-class",
    )
    write_text(paths["disallowed_class_payload"], canonical_json(disallowed_class_payload) + "\n")
    disallowed_class_response = send_sign_request(
        signer_proc,
        payload_path=paths["disallowed_class_payload"],
        signature_path=paths["scenario_dir"] / "disallowed_class_payload.sig",
        request_id="disallowed_class_request",
    )
    write_json(paths["disallowed_class_response"], disallowed_class_response)

    disallowed_target_payload = build_payload(
        label=label,
        scenario_name=scenario_name,
        key=key,
        gate_decision=gate_decision,
        scenario_verification_time=scenario_verification_time,
        request_name="disallowed_target_request",
        request_type="execution_receipt",
        payload_class="runtime_evidence",
        target_path="../outside/policy_violation.txt",
        content="wrong-target",
    )
    write_text(paths["disallowed_target_payload"], canonical_json(disallowed_target_payload) + "\n")
    disallowed_target_response = send_sign_request(
        signer_proc,
        payload_path=paths["disallowed_target_payload"],
        signature_path=paths["scenario_dir"] / "disallowed_target_payload.sig",
        request_id="disallowed_target_request",
    )
    write_json(paths["disallowed_target_response"], disallowed_target_response)

    write_json(
        paths["signer_request_log"],
        {
            "allowed_response": allowed_response,
            "disallowed_type_response": disallowed_type_response,
            "disallowed_class_response": disallowed_class_response,
            "disallowed_target_response": disallowed_target_response,
        },
    )

    runtime_report = {
        "report_version": 1,
        "report_type": "detached_signer_policy_runtime_report",
        "proof_label": label,
        "scenario": scenario_name,
        "status": "PASS",
        ...
        "allowed_signature_verified": allowed_verify["verified_ok"],
        "allowed_tamper_rejected": not allowed_tamper_verify["verified_ok"],
        "disallowed_type_rejected": not disallowed_type_response.get("ok", False),
        "disallowed_class_rejected": not disallowed_class_response.get("ok", False),
        "disallowed_target_rejected": not disallowed_target_response.get("ok", False),
        "disallowed_type_reason": disallowed_type_response.get("policy_reason"),
        "disallowed_class_reason": disallowed_class_response.get("policy_reason"),
        "disallowed_target_reason": disallowed_target_response.get("policy_reason"),
        "window_verdict": gate_decision["derived"]["window_verdict"],
        "notes": [
            "Signer فقط execution_receipt/runtime_evidence را sign می‌کند.",
            "request_type غیرمجاز رد شد.",
            "payload_class غیرمجاز رد شد.",
            "target_path خارج از policy boundary رد شد.",
        ],
    }
    ...
    return {
        ...
        "allowed_tamper_verify": allowed_tamper_verify,
        "disallowed_type_response": disallowed_type_response,
        "disallowed_class_response": disallowed_class_response,
        "disallowed_target_response": disallowed_target_response,
    }
...
def build_allowed_report(
    *,
    ...
    report = {
        ...
        "allowed_signature_verified": runtime_report["allowed_signature_verified"],
        "allowed_tamper_rejected": runtime_report["allowed_tamper_rejected"],
        "disallowed_type_rejected": runtime_report["disallowed_type_rejected"],
        "disallowed_class_rejected": runtime_report["disallowed_class_rejected"],
        "disallowed_target_rejected": runtime_report["disallowed_target_rejected"],
...
def build_denied_report(
    *,
    ...
    for key in (
        ...
        "allowed_tampered_payload",
        "allowed_tamper_verification",
        "disallowed_type_payload",
        "disallowed_type_response",
        "disallowed_class_payload",
        "disallowed_class_response",
        "disallowed_target_payload",
        "disallowed_target_response",
        "runtime_report",
        "executed_marker",
    ):
...
def run_signer_service(private_key_path: Path, key_id: str) -> int:
    ...
    def policy_check(payload: dict[str, Any]) -> tuple[bool, str | None]:
        if payload.get("payload_type") != "execution_receipt":
            return False, "disallowed_payload_type"
        if payload.get("payload_class") != "runtime_evidence":
            return False, "disallowed_payload_class"

        execution_output = payload.get("execution_output")
        if not isinstance(execution_output, dict):
            return False, "missing_execution_output"

        target_path = execution_output.get("target_path")
        if not isinstance(target_path, str):
            return False, "missing_target_path"

        normalized = os.path.normpath(target_path)
        if normalized.startswith("..") or os.path.isabs(normalized):
            return False, "disallowed_target_path"
    ...
    try:
        for line in sys.stdin:
            ...
            allowed, policy_reason = policy_check(payload)
            if not allowed:
                response = {
                    "ok": False,
                    "action": "sign",
...
def execute_allowed_scenario(
    *,
    ...
    bundle = simulate_policy_enforcement_execution(
        label=label,
        scenario_name=scenario_name,
        key=key,
        gate_decision=gate_decision,
    ...
    return {
        ...
        "disallowed_class_rejected": runtime_report["disallowed_class_rejected"],
        "disallowed_target_rejected": runtime_report["disallowed_target_rejected"],
        "disallowed_type_reason": runtime_report["disallowed_type_reason"],
        "disallowed_class_reason": runtime_report["disallowed_class_reason"],
        "disallowed_target_reason": runtime_report["disallowed_target_reason"],
        "runtime_report_exists": paths["runtime_report"].exists(),
...
def render_markdown(summary: dict[str, Any]) -> str:
    active = summary["scenarios"]["policy_runtime"]
    expired = summary["scenarios"]["expired_runtime"]
    future = summary["scenarios"]["future_runtime"]

    return f"""# R14 — Signer Policy Enforcement Proof + Request-Type / Payload-Class Restrictions
...
- Allowed signature verified: **{str(active.get("allowed_signature_verified", False)).upper()}**
- Allowed tamper rejected: **{str(active.get("allowed_tamper_rejected", False)).upper()}**
- Disallowed type rejected: **{str(active.get("disallowed_type_rejected", False)).upper()}**
- Disallowed class rejected: **{str(active.get("disallowed_class_rejected", False)).upper()}**
- Disallowed target rejected: **{str(active.get("disallowed_target_rejected", False)).upper()}**
- Disallowed type reason: `{active.get("disallowed_type_reason")}`
- Disallowed class reason: `{active.get("disallowed_class_reason")}`
- Disallowed target reason: `{active.get("disallowed_target_reason")}`
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="R14 - Signer Policy Enforcement Proof + Request-Type / Payload-Class Restrictions"
    )
    parser.add_argument(
        "--label",
        default="R14_detached_signer_policy_enforcement_proof",
        help="Proof label",
    )
    parser.add_argument(
        "--verification-time",
    ...
    output_dir = ARTIFACTS_DIR / "detached_external_signer_policy_proof" / args.label
    ...
    policy_key = ScenarioKey(
        key_id="r14-attestation-key-policy",
        private_key_path=custody_dir / "attestation_private_policy.pem",
        public_key_path=output_dir / "public_keys" / "attestation_public_policy.pem",
    ...
    expired_key = ScenarioKey(
        key_id="r14-attestation-key-expired",
    ...
    future_key = ScenarioKey(
        key_id="r14-attestation-key-future",
    ...
    for key in (policy_key, expired_key, future_key):
    ...
    registry = {
        "registry_version": 1,
        "registry_type": "detached_external_signer_policy_registry",
        "generated_at_utc": utc_now_iso(),
        "entries": [
            {
                "key_id": key.key_id,
    ...
    policy_runtime = execute_allowed_scenario(
        label=args.label,
        scenario_name=f"{args.label}__policy_runtime",
        key=policy_key,
    ...
    if not policy_runtime["gate_decision_allow"] or not policy_runtime["runtime_status_allow_executed"]:
        proof_status = "FAIL"
    if policy_runtime.get("detached_signer_key_path_exists_after_detach", True):
        proof_status = "FAIL"
    if policy_runtime.get("control_plane_private_key_read_allowed", True):
        proof_status = "FAIL"
    if not policy_runtime.get("allowed_signature_verified", False):
        proof_status = "FAIL"
    if not policy_runtime.get("allowed_tamper_rejected", False):
        proof_status = "FAIL"
    if not policy_runtime.get("disallowed_type_rejected", False):
        proof_status = "FAIL"
    if not policy_runtime.get("disallowed_class_rejected", False):
        proof_status = "FAIL"
    if not policy_runtime.get("disallowed_target_rejected", False):
        proof_status = "FAIL"
    if policy_runtime.get("disallowed_type_reason") != "disallowed_payload_type":
        proof_status = "FAIL"
    if policy_runtime.get("disallowed_class_reason") != "disallowed_payload_class":
        proof_status = "FAIL"
    if policy_runtime.get("disallowed_target_reason") != "disallowed_target_path":
        proof_status = "FAIL"
    ...
    summary = {
        "report_version": 1,
        "report_type": "detached_external_signer_policy_enforcement_proof",
        "generated_at_utc": utc_now_iso(),
    ...
    summary_json_path = output_dir / "detached_external_signer_policy_enforcement_proof.json"
    summary_md_path = output_dir / "detached_external_signer_policy_enforcement_proof.md"
    digest_path = output_dir / "detached_external_signer_policy_enforcement_proof_digest.json"
    ...
    digest = {
        ...
        "policy_enforcement_safe": (
            policy_runtime["gate_decision_allow"]
            and policy_runtime["runtime_status_allow_executed"]
            and not policy_runtime.get("control_plane_private_key_read_allowed", True)
            and not policy_runtime.get("detached_signer_key_path_exists_after_detach", True)
            and policy_runtime.get("allowed_signature_verified", False)
            and policy_runtime.get("allowed_tamper_rejected", False)
            and policy_runtime.get("disallowed_type_rejected", False)
            and policy_runtime.get("disallowed_class_rejected", False)
            and policy_runtime.get("disallowed_target_rejected", False)
        ),
    ...
    print("=" * 88)
    print("R14 - SIGNER POLICY ENFORCEMENT PROOF + REQUEST-TYPE / PAYLOAD-CLASS RESTRICTIONS")
    print("=" * 88)
    ...
    print(f"POLICY EXECUTED                             : {policy_runtime['runtime_status_allow_executed']}")
    print(f"ALLOWED VERIFIED                            : {policy_runtime.get('allowed_signature_verified', False)}")
    print(f"DISALLOWED TYPE REJECTED                    : {policy_runtime.get('disallowed_type_rejected', False)}")
    print(f"DISALLOWED CLASS REJECTED                   : {policy_runtime.get('disallowed_class_rejected', False)}")
    print(f"DISALLOWED TARGET REJECTED                  : {policy_runtime.get('disallowed_target_rejected', False)}")
