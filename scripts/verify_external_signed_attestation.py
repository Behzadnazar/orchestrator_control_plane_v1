...
VERIFY_ROOT = OPERATIONS_ROOT / "external_attestation_verification"
...
def build_markdown_report(result: dict, output_dir: Path) -> str:
    lines: list[str] = []
    lines.append("# External Signed Attestation Verification")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify external-key signed execution attestation with public key only.")
    ...
    public_key_path = require_file(ROOT / str(attestation["public_key_path"]))
    ...
    checks = {
        ...
        "public_key_hash_matches": sha256_file(public_key_path) == attestation.get("public_key_sha256"),
    ...
    result = {
        ...
        "verification_type": "external_signed_attestation_verification",
    ...
    json_path = output_dir / "external_signed_attestation_verification.json"
    md_path = output_dir / "external_signed_attestation_verification.md"
    digest_path = output_dir / "external_signed_attestation_verification_digest.json"
    ...
    print("=" * 72)
    print("EXTERNAL SIGNED ATTESTATION VERIFICATION")
    print("=" * 72)
