...
def state_file(project_root: Path) -> Path:
    path = project_root / "artifacts" / "state" / "phase11" / "current_workflow_run.json"
...
def save_current_workflow_run(project_root: Path, payload: Dict[str, Any]) -> None:
    ...
    state_file(project_root).write_text(
...
def load_current_workflow_run(project_root: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        raise RuntimeError("phase11 state file missing")
...
def fetch_rows_by_rowids(project_root: Path, rowids: List[int]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        rows = conn.execute(
            f"""
            SELECT rowid, task_id, task_type, status, attempt_count, last_error, priority
            FROM task_queue
...
def approve_row(project_root: Path, rowid: int, task_type: str) -> Tuple[bool, Dict[str, Any]]:
    res = run_cmd(
        ...
        [
            ...
            str(rowid),
            "--reason",
            f"phase11 onboarding approval for {task_type}",
        ],
    )
    ...
    return res.returncode == 0, payload
...
def main() -> int:
    ...
    seed = run_cmd(project_root, ["python3", "scripts/seed_phase11_workflow.py"])
