...
def build_manifest(
    *,
    ...
    return {
        "manifest_version": 1,
        "proof_type": "real_external_signature_verification_tamper_proof",
        "proof_label": label,
        "scenario": scenario_name,
        "generated_at_utc": utc_now_iso(),
        "workflow": {
            "name": "external_signed_runtime_gate",
            "mode": "real_external_signature_verification",
        },
    ...
    *,
    ...
    return {
        ...
        "checks": {
            ...
            "private_key_exists": key.private_key_path.exists(),
...
def openssl_verify(data_path: Path, public_key_path: Path, signature_path: Path) -> dict[str, Any]:
    ...
    verified_ok = "Verified OK" in combined and result.returncode == 0
    return {
        "command": [
            "openssl",
            "dgst",
...
def simulate_signed_runtime_execution(
    *,
    ...
    runtime_report = {
        "report_version": 1,
        "report_type": "real_external_signed_runtime_report",
        "proof_label": label,
        "scenario": scenario_name,
        "status": "PASS",
        ...
        "notes": [
            "Receipt با private key واقعی sign شد.",
            "Receipt با public key verify شد.",
            "Tampered receipt رد شد.",
            "Attestation با private key واقعی sign شد.",
            "Tampered attestation رد شد.",
        ],
    }
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    return f"""# R9 — Real External Signature Verification + Tamper Proof on Runtime Receipt/Attestation
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="R9 - Real External Signature Verification + Tamper Proof on Runtime Receipt/Attestation"
    )
    parser.add_argument(
        "--label",
        default="R9_real_external_signature_verification_tamper_proof",
        help="Proof label",
    )
    parser.add_argument(
        "--verification-time",
    ...
    output_dir = ARTIFACTS_DIR / "real_external_signature_verification_tamper_proof" / args.label
    ...
    active_key = ScenarioKey(
        key_id="r9-attestation-key-active",
        private_key_path=keys_dir / "attestation_private_active.pem",
        public_key_path=keys_dir / "attestation_public_active.pem",
        not_before=iso_no_microseconds(active_not_before),
        not_after=iso_no_microseconds(active_not_after),
    )
    expired_key = ScenarioKey(
        key_id="r9-attestation-key-expired",
        private_key_path=keys_dir / "attestation_private_expired.pem",
        public_key_path=keys_dir / "attestation_public_expired.pem",
        not_before=iso_no_microseconds(expired_not_before),
        not_after=iso_no_microseconds(expired_not_after),
    )
    future_key = ScenarioKey(
        key_id="r9-attestation-key-future",
        private_key_path=keys_dir / "attestation_private_future.pem",
        public_key_path=keys_dir / "attestation_public_future.pem",
    ...
    registry = {
        "registry_version": 1,
        "registry_type": "real_external_signature_test_registry",
        "generated_at_utc": utc_now_iso(),
        "entries": [
            {
                "key_id": key.key_id,
    ...
    summary = {
        "report_version": 1,
        "report_type": "real_external_signature_verification_tamper_proof",
        "generated_at_utc": utc_now_iso(),
    ...
    summary_json_path = output_dir / "real_external_signature_verification_tamper_proof.json"
    summary_md_path = output_dir / "real_external_signature_verification_tamper_proof.md"
    digest_path = output_dir / "real_external_signature_verification_tamper_proof_digest.json"
    ...
    print("=" * 72)
    print("R9 - REAL EXTERNAL SIGNATURE VERIFICATION + TAMPER PROOF")
    print("=" * 72)
