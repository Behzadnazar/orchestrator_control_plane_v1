...
def build_markdown_report(report: dict, output_dir: Path) -> str:
    lines: list[str] = []
    lines.append("# Signed Execution Entry")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Gate decision is mandatory before payload execution.")
    lines.append("- Payload receives signed-execution environment values from the wrapper.")
    lines.append("- A signed execution attestation is emitted for the runtime attempt.")
    lines.append("")
    lines.append("## Attestation")
    lines.append("")
    lines.append(f"- Attestation path: `{report['attestation_path']}`")
    lines.append(f"- Attestation signature: `{report['attestation_signature']}`")
    lines.append(f"- Signature verified at write-time: **{report['attestation_verified_at_write_time']}**")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Signed execution entry with admission gate enforcement and local HMAC attestation."
    )
    ...
    output_dir = SIGNED_RUNTIME_ROOT / runtime_label
    ...
    seed, attestation_key_hex = derive_attestation_key(
        workflow_id=workflow_id,
        operation=operation,
    ...
    payload_env["CONTROL_PLANE_ATTESTATION_SEED"] = seed
    payload_env["CONTROL_PLANE_ATTESTATION_KEY_SHA256"] = sha256_bytes(bytes.fromhex(attestation_key_hex))
    ...
    report_json_path = output_dir / "signed_runtime_report.json"
    report_md_path = output_dir / "signed_runtime_report.md"
    receipt_path = output_dir / "signed_execution_receipt.json"
    attestation_path = output_dir / "signed_execution_attestation.json"
    digest_path = output_dir / "signed_runtime_digest_report.json"
    ...
    receipt = {
        "receipt_version": 1,
        "receipt_type": "signed_execution_receipt",
    ...
    attestation_payload = {
        ...
        "attestation_type": "local_signed_execution_attestation",
        "generated_at_utc": generated_at,
        "runtime_label": runtime_label,
        "workflow_id": workflow_id,
        "operation": operation,
        ...
        "manifest_sha256": sha256_file(normalized_manifest_path),
        "gate_decision_sha256": sha256_file(gate_decision_path),
        "attestation_key_sha256": sha256_bytes(bytes.fromhex(attestation_key_hex)),
    }
    attestation_signature = sign_payload(attestation_key_hex, attestation_payload)
    attestation = {
        **attestation_payload,
        "signature_type": "hmac_sha256_local_attestation_v1",
        "signature": attestation_signature,
    }
    write_json(attestation_path, attestation)

    signature_verified = sign_payload(attestation_key_hex, attestation_payload) == attestation_signature

    report = {
        "report_version": 1,
        "report_type": "signed_execution_entry",
        "generated_at_utc": generated_at,
        "runtime_label": runtime_label,
        "workflow_id": workflow_id,
        "operation": operation,
        ...
        "attestation_verified_at_write_time": signature_verified,
        "security_boundary_mode": "wrapper_gate_plus_local_hmac_attestation",
        "mandatory_execution_env": {
            "CONTROL_PLANE_MANDATORY_EXECUTION": "1",
            "CONTROL_PLANE_SIGNED_EXECUTION": "1",
            ...
            "CONTROL_PLANE_ATTESTATION_SEED": seed,
            "CONTROL_PLANE_ATTESTATION_KEY_SHA256": sha256_bytes(bytes.fromhex(attestation_key_hex)),
        },
    }
