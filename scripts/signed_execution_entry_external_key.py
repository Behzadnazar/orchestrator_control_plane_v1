...
def run_shell(command: str, env: dict) -> dict:
...
def openssl_sign(private_key_path: Path, payload_bytes: bytes) -> str:
...
def build_markdown_report(report: dict, output_dir: Path) -> str:
    lines: list[str] = []
    lines.append("# External-Key Signed Execution Entry")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Trust Separation")
    lines.append("")
    lines.append("- Signing uses an external private key file.")
    lines.append("- Verification is expected to use only the public key.")
    lines.append("- The attestation artifact contains the public-key path, not private-key material.")
    lines.append("")
    lines.append("## Artifacts")
    lines.append("")
    lines.append(f"- Receipt path: `{report['receipt_path']}`")
    lines.append(f"- Attestation path: `{report['attestation_path']}`")
    lines.append(f"- Public key path: `{report['public_key_path']}`")
    lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Signed execution entry with external private key and trust-separated public-key verification model."
    )
    ...
    private_key_path = Path(str(manifest.get("private_key_path", "")).strip()).resolve()
    public_key_path = Path(str(manifest.get("public_key_path", "")).strip()).resolve()
    ...
    output_dir = EXTERNAL_SIGNED_ROOT / runtime_label
    ...
    payload_env["CONTROL_PLANE_SIGNED_EXECUTION"] = "1"
    payload_env["CONTROL_PLANE_TRUST_SEPARATED_SIGNING"] = "1"
    ...
    payload_env["CONTROL_PLANE_PUBLIC_KEY_PATH"] = str(public_key_path)

    if gate_value == "ALLOW":
        payload_executed = True
        payload_run = run_shell(command, env=payload_env)
    ...
    receipt_path = output_dir / "external_signed_execution_receipt.json"
    attestation_path = output_dir / "external_signed_execution_attestation.json"
    report_json_path = output_dir / "external_signed_runtime_report.json"
    report_md_path = output_dir / "external_signed_runtime_report.md"
    digest_path = output_dir / "external_signed_runtime_digest_report.json"
    ...
    attestation_payload = {
        ...
        "attestation_type": "external_key_signed_execution_attestation",
        "generated_at_utc": generated_at,
        "runtime_label": runtime_label,
        "workflow_id": workflow_id,
        "operation": operation,
        ...
        "payload_stdout_sha256": sha256_bytes(payload_stdout.encode("utf-8")),
        "payload_stderr_sha256": sha256_bytes(payload_stderr.encode("utf-8")),
        "payload_returncode": payload_returncode,
        "payload_executed": payload_executed,
        "manifest_sha256": sha256_file(normalized_manifest_path),
        "gate_decision_sha256": sha256_file(gate_decision_path),
        "public_key_path": rel(public_key_path),
        "public_key_sha256": sha256_file(public_key_path),
        "trust_boundary_mode": "external_private_key_signing_public_key_verification",
        "private_key_material_embedded": False,
    }
    signature_b64 = openssl_sign(private_key_path, canonical_json(attestation_payload).encode("utf-8"))
    attestation = {
        **attestation_payload,
        "signature_type": "rsa_sha256_openssl_external_key_v1",
    ...
    report = {
        "report_version": 1,
        "report_type": "external_key_signed_execution_entry",
        "generated_at_utc": generated_at,
        "runtime_label": runtime_label,
        "workflow_id": workflow_id,
        "operation": operation,
    ...
    digest_report = {
        ...
        "runtime_status": runtime_status,
        "artifacts": [
            {"path": rel(normalized_manifest_path), "size_bytes": normalized_manifest_path.stat().st_size, "sha256": sha256_file(normalized_manifest_path)},
            {"path": rel(receipt_path), "size_bytes": receipt_path.stat().st_size, "sha256": sha256_file(receipt_path)},
            {"path": rel(attestation_path), "size_bytes": attestation_path.stat().st_size, "sha256": sha256_file(attestation_path)},
            {"path": rel(report_json_path), "size_bytes": report_json_path.stat().st_size, "sha256": sha256_file(report_json_path)},
            {"path": rel(report_md_path), "size_bytes": report_md_path.stat().st_size, "sha256": sha256_file(report_md_path)},
            {"path": rel(stdout_path), "size_bytes": stdout_path.stat().st_size, "sha256": sha256_file(stdout_path)},
            {"path": rel(stderr_path), "size_bytes": stderr_path.stat().st_size, "sha256": sha256_file(stderr_path)},
        ],
    }
    write_json(digest_path, digest_report)

    print("=" * 72)
    print("EXTERNAL-KEY SIGNED EXECUTION ENTRY")
    print("=" * 72)
    ...
    print(f"ATTESTATION   : {rel(attestation_path)}")
