...
def cmd_hold_lock(args: argparse.Namespace) -> None:
    db.init_db()
    task_id = args.task_id
    agent_id = args.agent_id
    file_path = args.file_path
    ok, conflicts = db.acquire_file_locks(task_id, agent_id, [file_path])
    if ok:
        print(f"LOCK_ACQUIRED task_id={task_id} agent_id={agent_id} file={file_path}")
    else:
        print(f"LOCK_CONFLICT file={file_path} conflicts={conflicts}")

def cmd_release_lock(args: argparse.Namespace) -> None:
    db.init_db()
    db.release_file_locks(args.task_id)
    print(f"LOCK_RELEASED task_id={args.task_id}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("hold-lock")
    s.add_argument("--task-id", required=True)
    s.add_argument("--agent-id", required=True)
    s.add_argument("--file-path", required=True)
    s.set_defaults(func=cmd_hold_lock)

    s = sub.add_parser("release-lock")
    s.add_argument("--task-id", required=True)
